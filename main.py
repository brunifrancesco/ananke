...
from room import RoomContainers
...
@app.route('/summary')
def summary():
    return json.dumps({key_room: value.votes for key_room, value in RoomContainers().get_rooms()})
...
@app.route('/')
def index():
    room_key = ''.join(random.choice(string.ascii_lowercase) for i in range(8))
    return redirect("/rooms/%s" %room_key, code=302)
